...
function M.GameSceneReq(userid, _)
    ...
    p:send("csmj.GAME_EnterGameAck", enter_msg)
    ...
    p:send("csmj.GAME_GameSceneAck", scene_msg)

    -- 广播
    local broad = {
        userData = p:dump(match, false)
    }
    player_mgr:broadcast_but(p.userid,"csmj.GAME_PlayerEnterAck", broad)
end
...
function M.GAME_ReadyReq(userid, msg)
    ...
    player_mgr:broadcast("csmj.GAME_ReadyAck", broad)
...
function M.GAME_CallScoreReq(userid, msg)
    ...
    player_mgr:broadcast_but(p.userid, "csmj.GAME_CallScoreAck", broad)

    broad.cards = p.cards
    broad.type = p:get_card_type()
    p:send("csmj.GAME_CallScoreAck", broad)
...
function M.GAME_GameSuanNiuReq(userid, _)
    ...
    player_mgr:broadcast("csmj.GAME_GameSuanNiuAck", broad)
     -- 回放
    table.insert(match.record.game.data, {time = os.time(), name = "csmj.GAME_GameSuanNiuAck", msg = broad})
...
function M.register()
    M.tbl = {
        ...
        ["csmj.GAME_ReadyReq"] = M.GAME_ReadyReq,
        ["csmj.GAME_XiaZhuang"] = M.GAME_XiaZhuang,
        ["csmj.GAME_BeginReq"] = M.GAME_BeginReq,
        ["csmj.GAME_GameBankReq"] = M.GAME_GameBankReq,
        ["csmj.GAME_CallScoreReq"] = M.GAME_CallScoreReq,
        ["csmj.GAME_GameSuanNiuReq"] = M.GAME_GameSuanNiuReq,
    }
end
